...
class _FrontCameraState extends State<FrontCamera>
  ...
  Future<List<CameraDescription>> availableCamera() async {
    try {
  ...
  void _getAllAvialableCameraLens() {
    availableCameras().then((lstCameras) {
      if (lstCameras.isNotEmpty) {
        for (CameraDescription d in lstCameras) {
          final lens =
              EnumHandler.cameraLensDirectionToCameraLens(d.lensDirection);
  ...
  Future<void> _initCamera() async {
    availableCameras().then((lstCameras) async {
      if (lstCameras.isNotEmpty) {
        final cameras = lstCameras
            .where((c) =>
                c.lensDirection ==
  ...
  void _onTakePictureButtonPressed() async {
    ...
    try {
      if(Platform.isAndroid){
        AudioPlayer().play(AssetSource('shutter.wav'));
      }
