...
class CommonUtils {
  ...
  static bool isStartLoading = false;
  static String messageId = "0";
  ...
  logoutUser(BuildContext context) async {
    ...
    await userTbl.updateFirebaseToken(isRemovedToken: true);
    PreferenceUtils.removedAllKey();
    CommonUtils()
        .auth
        .signOut()
  ...
  String meterToKiloMeterConverter(int? meter) {
    if (meter != null) {
      double distanceInKiloMeters = meter / 1000;
      double roundDistanceInKM =
          double.parse((distanceInKiloMeters).toStringAsFixed(2));
      return '$roundDistanceInKM kilometers away';
    } else {
      return '';
    }
  }

  backFromScreenOrExit(bool isPrevious, String route) {
    if (isPrevious) {
      Get.toNamed(route, preventDuplicates: true);
    } else {
      if (Get.previousRoute != "/looking_for" &&
          Get.previousRoute != "/back_looking_for") {
        Get.back();
      } else {
  ...
  static bool isLinkedAllAccount() {
    final list = CommonUtils().auth.currentUser?.providerData;
    debugPrint("phoneNumber list   $list");
    List<String> providerList = [];
    if (list != null) {
      for (var data in list) {
        providerList.add(data.providerId);
      }
    }
    if (providerList.contains("google.com") &&
        providerList.contains("facebook.com") &&
        providerList.contains("apple.com") &&
        providerList.contains("phone")) {
      return true;
    }
    return false;
  }

  static bool isLinked(User user, String loginType) {
    List<String> providerList = [];
    final list = user.providerData;
    for (var data in list) {
      providerList.add(data.providerId);
    }
    if (loginType == "facebook" && providerList.contains("facebook.com")) {
      return true;
    } else if (loginType == "google" && providerList.contains("google.com")) {
      return true;
    }
    return false;
  }

  static bool isLinkedPhone() {
    if (CommonUtils().auth.currentUser != null) {
      final list = CommonUtils().auth.currentUser?.providerData;
      debugPrint("isLinkedPhone list ===> $list");
      List<String> providerList = [];
      if (list != null) {
        for (var data in list) {
          providerList.add(data.providerId);
  ...
  static Future<bool> checkEmailExits(String email) async {
    List<UserNewModel> lstAllUsers = <UserNewModel>[];
    ...
    await firebaseFirestore
        ...
        .get()
        .then((value) {
      List.from(value.docs.map(
          (user) => {lstAllUsers.add(UserNewModel.fromJsonMap(user.data()))}));
  ...
  Future<void> unLinkProvider(String unlinkProvider) async {
    List<UserInfo>? providerdata = CommonUtils().auth.currentUser?.providerData;
    for (UserInfo userInfo in providerdata!) {
      if (userInfo.providerId == unlinkProvider) {
        CommonUtils().auth.currentUser?.unlink(unlinkProvider);
      }
    }
  }

  String timestampToDate(Timestamp timestamp, String dateFormat) {
  ...
  /*void sendPushNotification(String userToken, String firebaseToken) async {
    try {
      ...
      header = {
        ...
        'Authorization': 'key=AIzaSyDVboDBrGE8yGYH5r761FKrnO9uUVzTrF8',
      };
      ...
      apiProvider
          .post(apiurl: ApiUrl.pushSend, header: header, body: requestParams)
          .then((value) {
        debugPrint("send notification ===> ${value.body}");
      });
    } catch (e) {
